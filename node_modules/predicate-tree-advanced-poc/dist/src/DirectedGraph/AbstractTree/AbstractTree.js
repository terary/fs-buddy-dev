...
class AbstractTree {
    constructor(rootNodeId = "_root_", nodeContent) {
        ...
        this._incrementor = new Incrementor_1.Incrementor();
    ...
    getNodeIdsWithNodeContent(matcherFn, shouldIncludeSubtrees //*tmc* what about subtrees ?
    ) {
    ...
    removeNodeAt(nodeId) {
        if (this.isRoot(nodeId)) {
            throw new DirectedGraphError_1.DirectedGraphError("Can not remove root node.");
        }
...
_AbstractTree_instances = new WeakSet(), _AbstractTree_appendChildNodeWithContent = function _AbstractTree_appendChildNodeWithContent(parentNodeId, nodeContent) {
    ...
    throw new DirectedGraphError_1.DirectedGraphError(`Tried to retrieve node that does not exist. nodeId: "${nodeId}".`);
    ...
    if (!nodeId) {
        throw new DirectedGraphError_1.DirectedGraphError(`Could not derive parent nodeId from '${nodeId}'.`);
    }
    let parentNodeId = "";
    parentNodeId = nodeId
    ...
    if (this.isRoot(nodeId) && nodeContent instanceof AbstractTree) {
        throw new DirectedGraphError_1.DirectedGraphError(`Can not replace root with subtree.`);
    }
