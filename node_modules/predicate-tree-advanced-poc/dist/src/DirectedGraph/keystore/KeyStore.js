...
class KeyStore {
    ...
    keyExistsOrThrow(key) {
        if (!this.keyExists(key)) {
            throw new KeyStoreError_1.KeyStoreError(`Key: '${key}' does not exist in store.`);
        }
        return true;
    }
    //putValue(value: T, newKey = getNextChildNodeId()): string {
    putValue(value, newKey = uuid_1.v4()) {
        if (this.keyExists(newKey)) {
            throw new KeyStoreError_1.KeyStoreError("Tried to overwrite value, key already exists in store. (use .replaceValue).");
        }
    ...
    reverseLookUpExactlyOneOrThrow(value) {
        const candidates = this.reverseLookUp(value);
        if (candidates.length !== 1) {
            throw new KeyStoreError_1.KeyStoreError(`Key: '${value}' has ${candidates.length} matches. Can not determine 1:1 mapping.`);
        }
        return candidates[0];
    }
    /**
